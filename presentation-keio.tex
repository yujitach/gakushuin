...
\begin{frame}
The only major change is that at that time only Kazuya was the collaborator, and the paper was unpublished....

\medskip

\centerline{\begin{tabular}{c@{\qquad\qquad}c}
\onslide<2>{\includegraphics[height=.4\textheight]{chang-tse.jpg}}& 
\includegraphics[height=.4\textheight]{kazuya.jpg} \\
\only<2>{Chang-Tse Hsieh} & Kazuya Yonekura \\
\only<2>{謝長澤} & 米倉和也 \\ 
\only<2>{\green{cond-mat}} & \green{hep-th} \\
\only<2>{IPMU/ISSP} & Tohoku
\end{tabular}}

\medskip

\onslide<2>{To complete it, the help of Chang-Tse was essential. Now it's a paper with three authors.
