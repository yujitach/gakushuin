...
\begin{array}{c|cccccccccc}
\text{\orange{mag.~charge}}& \cdots & -3 & -2 & -1 & 0 & +1 & +2 & +3 & \cdots \\
\hline
\text{$\b$ or $\f$ ?}& & \b & \b & \b & \b & \b & \b & \b & 
\end{array}
\]or  \[
...
\begin{itemize}
\item whether particles with elec.~charge $q \equiv 1 \mod 2$  is $\b$ or $\f$
\item whether particles with mag.~charge $m \equiv 1 \mod 2$  is $\b$ or $\f$
\end{itemize}
...
\boo{Global Gravitational Anomaly of\\[.5em]
the Maxwell theory}
...
\incx{pic17e}
\]
Consider the prototypical case of the integer quantum Hall effect: \hyperlink{detail}{\beamerbutton{details}}
\begin{itemize}
\item \blue{1+1}d boundary hosts \alert{gapless chiral fermion}
\item \blue{2+1}d bulk described by  \green{Chern-Simons}
\end{itemize}
...
Recently with Chang-Tse Hsieh (謝長澤) and Kazuya Yonekura (米倉和也),
we determined the anomaly of the  $SL(2,\mathbb{Z})$ electromagnetic duality of the Maxwell theory.

Thanks to the bulk-boundary correspondence, all have to do is to analyze
...
The point is that the 3+1d Maxwell theory is a $T^2$ comapctification of a 5+1d self-dual tensor theory.
...
\boo{On the chiral fermions on the \\[.5em]
boundary of the Chern-Simons theory}
